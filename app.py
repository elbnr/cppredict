...
st.title('Real Car Price ')
df = pd.read_csv("C:/Users/abhis/OneDrive/Desktop/quikr_car.csv")
st.dataframe(df) 
name1 = ['Hyundai Santro Xing', 'Mahindra Jeep CL550', 'Hyundai Grand i10',
...
with col1:
    company = st.selectbox("Select company",sorted (company))
with col2:
    name = st.selectbox("Select name",sorted (name1))
with col3:
    year = st.selectbox("Select year",sorted (year))

col3 , col4 = st.columns(2)
with col3:
    kms_driven = st.number_input("kms")
with col4:
    fuel_type = st.selectbox("Select Fuel Type",(fuel_type))


if st.button("Estimated price"):
    input_df = pd.DataFrame({"name":[name1],"company":[company],"year":[year],"kms_driven":[kms_driven],"fuel_type":[fuel_type]})
    result = pipe.predict(input_df)
    real_price = int(result)
